...
class Gazebo_Lab06_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        divisions = 10 
        width_size = width/divisions
        _, img_bin = cv2.threshold(cv_image, 100,255, cv2.THRESH_BINARY)
        small_chunk = img_bin[height-int(height/5):height][:]
        
        av = np.average(small_chunk, axis = 0) 
        min_value = np.amin(av)
        if min_value > 200:
            ...
            self.timeout = 0
            index_of_line = np.where(av == min_value)
            centre_min_index = np.average(index_of_line[0],0)
            high_state_index = int(centre_min_index/width_size)
            state[high_state_index] = 1
    ...
    def step(self, action):
        ...
        if not done:
            if action == 0:  # FORWARD
                ...
                max_ind = np.where(state == 1) 
                if max_ind == 4 or max_ind == 5 :
                    reward = 6
                elif max_ind == 3 or max_ind == 6 :
                    reward = 1
                else:
                    reward = 1
            elif action == 1:  # LEFT
                reward = 3
                max_ind = np.where(state == 1) 
                if max_ind == 0 or max_ind == 1 or max_ind == 2 or max_ind == 3:
                    reward = 5
                else:
                    reward = 1
            else: # RIGHT
                max_ind = np.where(state == 1) 
                if max_ind == 7 or max_ind == 8 or max_ind == 9 or max_ind == 6:
                    reward = 5
                else:
                    reward = 1
